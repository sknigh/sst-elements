...
#ifndef COMPONENTS_SNOTRA_RVMA_NIC_CMDS_H
#define COMPONENTS_SNOTRA_RVMA_NIC_CMDS_H
...
class NicCmd : public SST::Event {
  public:
    ...
    NicCmd() : Event() {}
	NicCmd( Type type ) : type( type ) {}

    void serialize_order(SST::Core::Serialization::serializer &ser)  override {
        Event::serialize_order(ser);
		ser & type;
	}
    ImplementSerializable(SST::Snotra::RVMA::NicCmd);
};
...
class PutCmd : public NicCmd {
  public:
	...
	PutCmd( Hermes::MemAddr srcAddr, size_t size, Hermes::RVMA::ProcAddr proc, Hermes::RVMA::VirtAddr virtAddr, size_t offset ) :
		NicCmd(Put), srcAddr(srcAddr), size(size), proc(proc), virtAddr(virtAddr), offset( offset )
	{}

	Hermes::MemAddr srcAddr;
	size_t size;
	Hermes::RVMA::ProcAddr proc;
