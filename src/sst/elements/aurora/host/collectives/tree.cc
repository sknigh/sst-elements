...
using namespace Aurora::Collectives;

Tree::Tree( int my_pe, int num_pes, int requested_crossover, int requested_radix):
    m_my_pe( my_pe ), m_num_pes(num_pes), m_debug(false)
{
    ...
    m_radix = requested_radix;
    m_crossover = requested_crossover;
    ...
    /* initialize the binomial tree for collective operations over
       entire tree */
    m_num_children = 0;
    for (i = 1 ; i <= m_num_pes ; i *= m_radix) {
        tmp_radix = (m_num_pes / i < m_radix) ?
            (m_num_pes / i) + 1 : m_radix;
    ...
    m_children.resize( m_num_children );
...
void Tree::build_kary_tree(int radix, int PE_start, int stride,
...
int Tree::circular_iter_next(int curr, int PE_start, int logPE_stride, int PE_size)
{
